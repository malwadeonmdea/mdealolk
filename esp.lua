...
local ESPEnabled = true -- Flag to track if ESP is enabled or disabled
...
game.Players.PlayerAdded:Connect(function(newplr)
    if ESPEnabled then -- Check if ESP is enabled before starting for new player
        local Top = Drawing.new("Line")
        local Bottom = Drawing.new("Line")
        local Left = Drawing.new("Line")
        local Right = Drawing.new("Line")
...
-- Add a stop box to toggle ESP on/off
local StopBox = MainTab:CreateToggle({
    Name = "Stop ESP",
    CurrentValue = false,
    Flag = "StopESPFlag",
    Callback = function(Value)
        ESPEnabled = not Value -- Update the flag to track if ESP is enabled
        if Value then
            stopESP() -- If toggled off, stop ESP
        else
            startESP() -- If toggled on, start ESP
        end
    end,
})
